...
| Supplementary Sources | `olist_profile.txt`, `docs/data_profile.json` |
| Date | 2026-03-12 |
...
Open Items: NONE — all items resolved per ASMP-001 (plugin selection, encoding, table naming, write disposition).
...
Open Items: (1) Credential provisioning — **Owner: Platform Engineer** to coordinate before implementation begins. `GOOGLE_APPLICATION_CREDENTIALS` must be set before any pipeline stage can run. Dataset naming resolved — `olist_raw` / `olist_analytics` confirmed; see ASMP-001.
...
Open Items: (1) `date_key` in `fct_sales` is derived from `order_purchase_timestamp` — the only timestamp guaranteed non-null across all order statuses and representing the moment the business transaction was committed. Data engineer may override with documented rationale per ASMP-004. (2) `order_payment_value` has been removed from `fct_sales` — order-level aggregate on an item-level fact; causes silent double-counting in multi-item orders. Payment totals per order are available from `fct_payments` directly. Document in `changelog.md`. `date_key` type confirmed as DATE (see ADR-001; Closed 2026-03-11).
...
Open Items: `dim_date` generation confirmed: `dbt_utils.date_spine(datepart="day", start_date="cast('2016-01-01' as date)", end_date="cast('2018-12-31' as date)")`. `sources.yml` must reference BigQuery `olist_raw` table names exactly as produced by Meltano per ASMP-001 — verify before running `dbt build`.
...
Open Items: NONE — `date_key` range test confirmed: `expect_column_values_to_be_between(column_name='date_key', min_value='2016-01-01', max_value='2018-12-31')`. Closed 2026-03-11.
...
| ASMP (Assumption) | 28 | All |

**Priority distribution (active requirements only):**
- P0: 38 requirements
- P1: 19 requirements
- P2: 3 requirements (Optional — REQ-062.1, REQ-063.1, REQ-064.1)
...
|---|---|---|---|
| 5.0 | 2026-03-12 | Patrick — Requirements Analyst; Claude — Platform Engineer | BRD finalized and closed. Status changed Draft → Final/Closed. Resolved Open Items converted from strikethrough to closed notes. Supplementary Sources updated to reflect actual files. ASMP count corrected (23→28). Foreword role-name note removed (superseded by v3.4). P2 count corrected in Completeness Summary. Revision history v2.9 row formatting fixed. |
...
*End of Document — BRD Project Caravela — Olist E-Commerce Analytics Pipeline v5.0 Final*
